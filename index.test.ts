...
describe('parseError', () => {
  ...
  test('firefox', () => {
    expect(parseError(initError(fixtureFirefoxStack))).toEqual({ message: '', stack: correct })
  })
  test('chrome', () => {
    expect(parseError(initError(fixtureStack))).toEqual({ message: 'Error raised', stack: correct })
  })
})
